<template>
  <div class="page-container">
	...
	<el-row align="center">
	  <el-col :span="24">
		  两队近<el-link type="danger" style="font-size:18px;">{{homeAndAwayTeamScoreList.length}}</el-link>场交锋,
		  主队<el-link type="danger" style="font-size:16px;" v-if="footballScore != null">{{footballScore.homeFootballTeamName}}</el-link>
		  胜场<el-link type="danger" style="font-size:16px;">{{homeAndAwayTeamScoreWinTotal}}</el-link>,
		  平场<el-link type="primary" style="font-size:16px;">{{homeAndAwayTeamScoreDrawTotal}}</el-link>,
		  负场<el-link type="success" style="font-size:16px;">{{homeAndAwayTeamScoreLostTotal}}</el-link>, 
		  胜率<el-link type="danger" style="font-size:16px;">{{homeAndAwayTeamScoreWinPercent}}%</el-link>
	  </el-col>
	</el-row>
	<el-row>
	  <el-col :span="24">
	...
	<el-row>
	  ...
	  <el-col :span="12" align="left">
		  <el-form :inline="true" :model="filters" :size="size">
			<font style="color:red;font-size:16px;">全部赛事</font>&nbsp;&nbsp;&nbsp;&nbsp;
			<el-form-item label="">
	...
	<el-row>
	  <el-col :span="12">
		<el-table :data="homeTeamScoreList" :highlight-current-row="true"
			v-loading="homeLoading" :element-loading-text="$t('action.loading')" :border="false" :stripe="true"
			:show-overflow-tooltip="true" align="left" size="mini" style="width:100%;" >
	  ...
	  <el-col :span="12">
		<el-table :data="awayTeamScoreList" :highlight-current-row="true"
			v-loading="awayLoading" :element-loading-text="$t('action.loading')" :border="false" :stripe="true"
			:show-overflow-tooltip="true" align="left" size="mini" style="width:100%;" >
		<el-table-column type="index" width="50" label="序号" align="center"></el-table-column>
		<el-table-column prop="footballLeagueMatchName" width="140" label="赛事" align="center"></el-table-column>
		<el-table-column prop="matchDate" width="90" label="时间" :formatter="dateFormat" align="center"></el-table-column>
...
export default {
	...
	data() {				
		return {
			...
			awayTeamScoreList: [],
			footballScore: null,
			homeAndAwayTeamScoreWinTotal: 0,
			homeAndAwayTeamScoreDrawTotal: 0,
			homeAndAwayTeamScoreLostTotal: 0,
			homeAndAwayTeamScoreWinPercent: 0.00,
			homeTeamScoreWinTotal: 0,
			homeTeamScoreDrawTotal: 0,
			homeTeamScoreLostTotal: 0,
			homeTeamScoreWinPercent: 0.00,	
			awayTeamScoreWinTotal: 0,
			awayTeamScoreDrawTotal: 0,
			awayTeamScoreLostTotal: 0,
			hawayTeamScoreWinPercent: 0.00,					
			loading: false, //加载标志
			homeLoading: false,
			awayLoading: false,
	...
	methods: {
		...
		findHomeAndAwayTeamScoreList: function (param) {
			...
			if(null == homeAndAwayTeamScoreQueryParams){
				...
				if(objectValueEqual(param,homeAndAwayTeamScoreQueryParams)){
					...
					this.computeHomeAndAwayTeamScoreFields();
				}else{
					this.findHomeAndAwayTeamScoreListCommon(param);
				}
			}
		...
		findHomeTeamScoreList: function (param) {
			...
			if(null == homeTeamScoreQueryParams){
				...
				if(objectValueEqual(param,homeTeamScoreQueryParams)){
					...
					this.computeHomeTeamScoreFields();
				}else{
					this.findHomeTeamScoreListCommon(param);
				}
			}
		...
		findAwayTeamScoreList: function (param) {
			...
			if(null == awayTeamScoreQueryParams){
				...
				if(objectValueEqual(param,awayTeamScoreQueryParams)){
					...
					this.computeAwayTeamScoreFields();
				}else{
					this.findAwayTeamScoreListCommon(param);
				}
			}
		...
		computeHomeAndAwayTeamScoreFields: function () {
			this.homeAndAwayTeamScoreWinTotal = 0;
			this.homeAndAwayTeamScoreDrawTotal = 0;
			this.homeAndAwayTeamScoreLostTotal = 0;
			this.homeAndAwayTeamScoreWinPercent = 0.00;
			if(this.homeAndAwayTeamScoreList.length > 0){
				for (let i = 0, len = this.homeAndAwayTeamScoreList.length; i < len; i++) {
					let score = this.homeAndAwayTeamScoreList[i];
					if(score.homeScore - score.awayScore > 0){
						this.homeAndAwayTeamScoreWinTotal = this.homeAndAwayTeamScoreWinTotal + 1;
					} else if(score.homeScore - score.awayScore < 0){
						this.homeAndAwayTeamScoreDrawTotal = this.homeAndAwayTeamScoreDrawTotal + 1;
					} else {
						this.homeAndAwayTeamScoreLostTotal = this.homeAndAwayTeamScoreLostTotal + 1;
					}
				}
				this.homeAndAwayTeamScoreWinPercent = accDiv(this.homeAndAwayTeamScoreWinTotal,this.homeAndAwayTeamScoreList.length,2) * 100;					
			}			
		},	
		computeHomeTeamScoreFields: function () {
			this.homeTeamScoreWinTotal = 0;
			this.homeTeamScoreDrawTotal = 0;
			this.homeTeamScoreLostTotal = 0;
			this.homeTeamScoreWinPercent = 0.00;
			if(this.homeTeamScoreList.length > 0){
				for (let i = 0, len = this.homeTeamScoreList.length; i < len; i++) {
					let score = this.homeTeamScoreList[i];
					if(score.homeScore - score.awayScore > 0){
						this.homeTeamScoreWinTotal = this.homeTeamScoreWinTotal + 1;
					} else if(score.homeScore - score.awayScore < 0){
						this.homeTeamScoreDrawTotal = this.homeTeamScoreDrawTotal + 1;
					} else {
						this.homeTeamScoreLostTotal = this.homeTeamScoreLostTotal + 1;
					}
				}
				this.homeTeamScoreWinPercent = accDiv(this.homeTeamScoreWinTotal,this.homeTeamScoreList.length,2) * 100;					
			}			
		},
		computeAwayTeamScoreFields: function () {
			this.awayTeamScoreWinTotal = 0;
			this.awayTeamScoreDrawTotal = 0;
			this.awayTeamScoreLostTotal = 0;
			this.awayTeamScoreWinPercent = 0.00;
			if(this.awayTeamScoreList.length > 0){
				for (let i = 0, len = this.awayTeamScoreList.length; i < len; i++) {
					let score = this.awayTeamScoreList[i];
					if(score.homeScore - score.awayScore > 0){
						this.awayTeamScoreWinTotal = this.awayTeamScoreWinTotal + 1;
					} else if(score.homeScore - score.awayScore < 0){
						this.awayTeamScoreDrawTotal = this.awayTeamScoreDrawTotal + 1;
					} else {
						this.awayTeamScoreLostTotal = this.awayTeamScoreLostTotal + 1;
					}
				}
				this.awayTeamScoreWinPercent = accDiv(this.awayTeamScoreWinTotal,this.awayTeamScoreList.length,2) * 100;					
			}			
		},				
